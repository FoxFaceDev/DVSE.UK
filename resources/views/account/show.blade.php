        ...
        <section class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            <form method="POST" action="{{ route('account.profile.update') }}" class="p-5 space-y-4">
                ...
                @unless($user->isInstructor())
                    <div><label for="account-language" class="block text-sm font-semibold text-gray-700 mb-1.5">Study language</label><select id="account-language" name="preferred_language_id" required class="w-full min-h-12 px-4 py-3 rounded-xl bg-gray-50 border border-gray-200 focus:bg-white focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none @error('preferred_language_id') border-red-500 @enderror">@foreach($languages as $language)<option value="{{ $language->id }}" @selected((string) old('preferred_language_id', $user->preferred_language_id) === (string) $language->id)>{{ $language->code === 'en' ? 'English only' : $language->name.' with English' }}</option>@endforeach</select><p class="mt-1.5 text-xs text-gray-500">This setting controls translations throughout learning sections and topics.</p>@error('preferred_language_id') <p class="mt-1.5 text-sm text-red-600">{{ $message }}</p> @enderror</div>
                @endunless
                <div>
