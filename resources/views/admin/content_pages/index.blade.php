<x-layouts.admin title="Learning Pages">
    ...
    <form method="GET" action="{{ route('admin.content-pages.index') }}" class="mb-5 rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
        ...
        <select name="topic_id" aria-label="Filter by topic" class="rounded-md border-gray-300 text-sm focus:border-primary focus:ring-primary">
            <option value="">All topics</option>
            @foreach($topics as $topic)
                <option value="{{ $topic->id }}" @selected((string) request('topic_id') === (string) $topic->id)>{{ $topic->name_en }} ({{ class_basename($topic->topicable_type) }})</option>
            @endforeach
        </select>
        ...
        @if(request()->hasAny(['q', 'topic_id', 'type', 'sort']))
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse($contentPages as $contentPage)
                    <tr class="hover:bg-gray-50/70">
                        ...
                        <td class="px-6 py-4 font-medium text-gray-900">{{ $contentPage->topic->name_en }}</td>
                        <td class="px-6 py-4">
