...
<body x-data="{ sidebarOpen: false }" class="bg-surface text-on-surface antialiased min-h-screen flex flex-col font-body">
    ...
    <header class="bg-white/80 backdrop-blur-md sticky top-0 z-40 border-b border-gray-200">
        <div class="max-w-md mx-auto px-4 py-3 flex items-center justify-between">
            ...
            <div>
                <button @click="sidebarOpen = !sidebarOpen" class="p-2 text-primary focus:outline-none">
    ...
    <div x-show="sidebarOpen" 
        ...
        <nav class="flex-1 py-4 flex flex-col font-medium text-secondary">
            @guest('web')
                <a href="{{ route('login') }}" class="px-6 py-3 hover:bg-surface-dim hover:text-primary transition-colors">Login</a>
                <a href="{{ route('register') }}" class="px-6 py-3 hover:bg-surface-dim hover:text-primary transition-colors">Register</a>
            @else
                <div class="px-6 py-4 mb-2 bg-primary/5 border-b border-primary/10">
                ...
                <a href="#" class="px-6 py-3 hover:bg-surface-dim hover:text-primary transition-colors">My Profile</a>
                <a href="{{ route('history') }}" class="px-6 py-3 hover:bg-surface-dim hover:text-primary transition-colors">My Test History</a>
            ...
            <div class="mt-auto px-6 py-4">
                <form action="{{ route('logout') }}" method="POST">
                    @csrf
                    <button type="submit" class="w-full py-2.5 bg-red-50 hover:bg-red-100 text-red-600 rounded-lg font-medium transition-colors">
                        Sign Out
                    </button>
                </form>
            </div>
