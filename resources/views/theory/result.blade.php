<x-layouts.app :showBack="false" title="Test Result">
    <div x-data="{
        ...
        <div x-cloak x-show="reviews.length" class="mx-auto w-full max-w-2xl text-left">
            <button type="button" @click="showWrongAnswers = !showWrongAnswers" :aria-expanded="showWrongAnswers" class="flex w-full items-center justify-between rounded-xl bg-gradient-to-r from-red-600 to-rose-500 px-5 py-4 font-bold text-white shadow-lg shadow-red-200 transition hover:-translate-y-0.5 hover:shadow-xl">
                <span class="flex items-center gap-3"><span class="flex h-9 w-9 items-center justify-center rounded-full bg-white/20"><svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v4m0 4h.01M10.3 3.6 2.5 17.1A2 2 0 0 0 4.2 20h15.6a2 2 0 0 0 1.7-2.9L13.7 3.6a2 2 0 0 0-3.4 0Z"/></svg></span><span x-text="showWrongAnswers ? 'Hide wrong answers' : `See wrong answers (${reviews.length})`"></span></span>
            ...
            <div x-cloak x-show="showWrongAnswers" x-transition class="mt-5 space-y-4">
